...
def remove_cart_item(request, cart_item_id):
    ...
    Args:
        request: Django request object
        cart_item_id: ID of CartItem to remove
        
    Returns:
        CartItem instance that was deleted, or None if not found
    """
    try:
        cart = get_or_create_cart(request)
...
def get_cart_items_with_breakdown(request):
    ...
    Useful for displaying cart summary to user.
    
    Args:
        request: Django request object
        
    Returns:
        Dictionary with items and summaries
    """
    cart = get_or_create_cart(request)
    items = []
