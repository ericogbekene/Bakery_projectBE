...
class DeliveryZone(models.Model):
    """
    Admin-managed delivery fee per area.

    Areas are entered freely by admin (no fixed list) — this lets you add,
    rename, or deactivate any neighborhood in and around FCT without a
    code change. Only zones with status='active' are shown to frontend
    users as delivery options; everywhere else, pickup is the only choice.
    """
    ...
    area_name = models.CharField(
        max_length=100,
        unique=True,
        help_text="Neighborhood/area this fee applies to, e.g. 'Wuse II', 'Karu'"
    )
    ...
    def __str__(self):
        return f"{self.area_name} - ₦{self.fee:,.0f}"
...
class DeliverySchedule(models.Model):
    ...
    def __str__(self):
        return f"{self.zone.area_name} - {self.get_day_of_week_display()} {self.time_slot_name} ({self.start_time.strftime('%H:%M')}-{self.end_time.strftime('%H:%M')})"
...
class DeliveryService:
    ...
    @staticmethod
    def calculate_delivery_fee(area_name, is_pickup=False):
        """
        Calculate delivery fee based on area name.
        Pickup orders always return a fee of 0 with no zone lookup.

        Args:
            area_name: Free-text area name (case-insensitive match against
                       active DeliveryZone entries)
        ...
        try:
            zone = DeliveryZone.objects.get(
                area_name__iexact=area_name.strip(),
                status='active'
            )
            return {
                'fee': zone.fee,
