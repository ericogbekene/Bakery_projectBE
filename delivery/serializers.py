...
class DeliveryZoneListSerializer(serializers.ModelSerializer):
    ...
    Used to populate the frontend area dropdown at checkout.
    """

    class Meta:
        model = DeliveryZone
        fields = ['id', 'area_name', 'fee']
...
class DeliveryScheduleSerializer(serializers.ModelSerializer):
    ...
    zone_area = serializers.CharField(source='zone.area_name', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            'id', 'zone', 'zone_area', 'day_of_week', 'day_display',
...
class DeliveryExceptionSerializer(serializers.ModelSerializer):
    ...
    zone_areas = serializers.SerializerMethodField()

    class Meta:
        model = DeliveryException
        fields = [
            'id', 'date', 'exception_type', 'exception_type_display',
            'title', 'description', 'zones', 'zone_areas',
            'delivery_available', 'modified_fee', 'modified_schedule'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at']

    def get_zone_areas(self, obj):
        """Return list of zone area names."""
        return [zone.area_name for zone in obj.zones.all()]


class CalculateDeliveryFeeSerializer(serializers.Serializer):
    """
    Serializer for calculating delivery fee by area.
    """
